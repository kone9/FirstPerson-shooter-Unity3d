...
namespace Assets.Scripts.Player
{
    public class PlayerShooting : MonoBehaviour
    {
		...
		private void Update()
		{
			...
			// NUEVO: disparar con click izquierdo del mouse
			bool mouseShoot = Input.GetMouseButton(0);

			// Mantengo la lógica original del joystick
			bool joystickShoot = JoysticShoot.IsShooting();

			// Disparar si:
			// - Hay click izquierdo
			// - O el joystick está disparando
			// - Y el tiempo entre disparos se cumplió
			if ((mouseShoot || joystickShoot) && _timer >= TimeBetweenBullets)
				Shoot();
